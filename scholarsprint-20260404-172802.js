...
const renderQuestionList = (knowledge = state.qaKnowledge || state.knowledge) => {
  ...
  if (!questions.length) {
    app.questionList.innerHTML = `
      <div class="empty-state">
        Add a source to load suggested questions here.
      </div>
    `;
    return;
  }
...
const resetWorkspace = () => {
  ...
  setStatus(
    app.generatorStatus,
    "Using your current source unless you add files or text here.",
    "idle"
  );
  setStatus(
    app.qaStatus,
...
const handleIngest = async ({ useSample = false } = {}) => {
  ...
  renderSourceOverview();
  renderQuestionList();
  setStatus(app.ingestStatus, "Importing your source...", "working");
...
const handleGenerateArtifact = async ({ silent = false } = {}) => {
  const sourceResult = await resolveKnowledgeFromInputs({
    emptyMessage:
      "Add source files or text before generating outputs.",
  ...
  if (!silent) {
    const sourceLabel =
      sourceResult.origin === "direct" ? "from this tab" : "from your current source";
...
const handleRefreshQuestions = async () => {
  const sourceResult = await resolveKnowledgeFromInputs({
    emptyMessage:
      "Add source files or text before refreshing questions.",
  ...
  const sourceLabel =
    sourceResult.origin === "direct" ? "this tab" : "your current source";
  setStatus(
    app.qaStatus,
...
const handleAnswerQuestion = async () => {
  const sourceResult = await resolveKnowledgeFromInputs({
    emptyMessage:
      "Add source files or text before answering questions.",
    files: Array.from(app.qaSourceFiles.files || []),
    statusElement: app.qaStatus,
    text: app.qaSourceText.value,
    textName: "Q&A source",
...
const buildGradeReport = (submissionText, rubricText, knowledge) => {
  ...
  return {
    ...
    markdown: [
      ...
      ...gaps.map((item) => `- ${item}`),
      `## Summary`,
      `This grade estimates how closely the submission matches the reference material. It is strongest when the response uses the same core vocabulary, explains connections clearly, and covers the main ideas.`,
    ].join("\n"),
    score: weightedScore,
    strengths,
...
const handleGradeSubmission = async () => {
  const sourceResult = await resolveKnowledgeFromInputs({
    emptyMessage:
      "Add reference files or text before grading.",
